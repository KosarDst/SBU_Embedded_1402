...
//void loop()
//{
 // int a=analogRead(A0);//any sort of signal on A0 with analogRead function read.
  //int b=map(a,0,1023,0,255);//quantization of signal,0,255 for 5V va mehvar y,0,1023 mehvar x
  //Serial.println(b);
  //delay(1000);
  
  //started to produce ultrasonic
...
/*
#define CLK_PIN 7      
#define ANTI_CLK_PIN 8

void setup(){
	pinMode(CLK_PIN,OUTPUT); //set pin7 --> Output
	pinMode(ANTI_CLK_PIN,OUTPUT); //set pin8 --> Output
}

void loop(){
	digitalWrite(CLK_PIN,HIGH);
	digitalWrite(ANTI_CLK_PIN,LOW);
	delay(5000); // rotate clock wise
	digitalWrite(CLK_PIN,LOW);
	digitalWrite(ANTI_CLK_PIN,LOW);
	delay(2000); // freez for 2 seconds
	digitalWrite(ANTI_CLK_PIN,HIGH);
	digitalWrite(CLK_PIN,LOW);
	delay(5000); // rotate anti clock wise
	digitalWrite(CLK_PIN,LOW);
	digitalWrite(ANTI_CLK_PIN,LOW);
	delay(2000); // freez for 2 seconds
}
